...
public class DetectExtension 
{
	public static void DetectExt(RandomAccessFile inStream) throws IOException
	{
		...
		else if(Main.inputExtension.toLowerCase().equals(".m4b"))
		{
			M4B_MystIV.M4BExtract(inStream);
			return;
		}
		else if(Main.inputExtension.toLowerCase().equals(".akb"))
		{
			AKB_FFIV.AKBextract(inStream);
			return;
		}
		else if(Main.inputExtension.toLowerCase().equals(".he4"))
		{
			HE4_Humongous.HE4extract(inStream);
			return;
		}
		else if(Main.inputExtension.toLowerCase().equals(".dmu"))
		{
			HE4_Humongous.DMUextract(inStream);
			return;
		}
		
		else if(Main.inputExtension.toLowerCase().equals(".mpo"))
		{
		...
		else
		{
			...
			{
				Main.inStream.seek(32);
				byte[] test = new byte[3];
				Main.inStream.read(test);
			...
			///////files with no header
				...
				int header = Helpers.readIntLittleEndian(Main.inStream);

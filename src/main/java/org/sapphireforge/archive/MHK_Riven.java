...
public class MHK_Riven 
{
	...
	{////////////////big endian
		...
		{///////////////multiple resorce types will overwrite
			...
			if (Main.arg.verbose) {System.out.println(fileTypeS);}
			...
			if (Main.arg.verbose) {System.out.println(Arrays.toString(idArray));}
			if (Main.arg.verbose) {System.out.println(Arrays.toString(indexArray));}
			...
			for (int j = 0; j < fileTableCount; j++)
			{
				...
				if (Main.arg.verbose) {System.out.println("File length: " + fileLength[j]);}
			...
			{
				//do proper names
				Output.OutSetup(Main.inputWithoutExtension + Main.separator + String.valueOf(idArray[j]),".png");
				...
				try
				{
					Main.outStream.write(MHK_Riven_Media.MHK_BMP_Convert(inStream, fileLength[j], fileOff[j]));
				}

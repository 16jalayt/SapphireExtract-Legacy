...
public class FAT_Windows
{
    public static void FATextract(RandomAccessFile inStream) throws IOException
    {
        ...
        inStream.seek(0);
        if(Main.arg.verbose) System.out.println("Searching for start of partition...");
        ...
        while(true)
        {
            try
            {
                ...
                {
                    if(Main.arg.verbose) System.out.println("Found 0xEB (first marker)");
                    inStream.skipBytes(1);
                    byte testByte2 = inStream.readByte();
                    if(testByte2==(byte)0x90)
                    {
                        if(Main.arg.verbose) System.out.println("Found 0x90 (second marker). Partition start found.");
                        break;
                    }
                }
            }
        ...
        if(Main.arg.verbose) System.out.println("Fat image formatted by: " + new String(BS_OEMName));
        ...
        if(Main.arg.verbose) System.out.println("Sectors per unit: " +  BPB_SecPerClus);

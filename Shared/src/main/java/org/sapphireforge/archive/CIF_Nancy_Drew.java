...
//External cif files seem different. Garbage data
	...
	private static void parseFile(RandomAccessFile inStream)throws IOException
	{
		...
		Output.OutSetup(ParseInput.inputWithoutExtension + ParseInput.separator + "out",".raw");
		...
		Output.OutSetup(ParseInput.inputWithoutExtension + ParseInput.separator + "out",".shift");
		ParseInput.outStream.write(fileraw);

		fileraw= DecompressionManager.decompressLZSS(fileraw);

		Output.OutSetup(ParseInput.inputWithoutExtension + ParseInput.separator + "out",".lszz");
		ParseInput.outStream.write(fileraw);
		System.exit(0);
	}

...
public class AVF_Nancy_Drew
{
    public static void AVFextract(RandomAccessFile inStream) throws IOException
    {
        ...
        for (int i = 0; i < numFrames; i++)
        {
            ...
            if(numFrames>1)
            {
                ...
                File outfile = new File(ParseInput.inputWithoutExtension + ParseInput.separator + frameNumber + ".png");
                outfile.getParentFile().mkdirs();
                ImageIO.write(img, "png", outfile);
            }
            else
            {
                File outfile = new File(ParseInput.inputWithoutExtension + ".png");
                ImageIO.write(img, "png", outfile);
            }
    ...
    {
        String outputName = ParseInput.inputWithoutExtension + ".mp4";

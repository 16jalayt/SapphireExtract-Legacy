...
public class CSSDAT_RCT 
{
	public static void DATextract(RandomAccessFile inStream) throws IOException
	{
		...
		if (ParseInput.inputWithoutExtension.toLowerCase().compareTo("css1") == 0)
		{	//import text file if css1.dat to lookup names
			...
			for (int i = 0; i < numfiles; i++)
			{
				...
				Output.OutSetup(ParseInput.inputWithoutExtension + ParseInput.separator + FileList[i],".wav");
		...
		else
		{
			...
			Output.OutSetup(ParseInput.inputWithoutExtension,".wav");
			ParseInput.outStream.write(fileout);
			ParseInput.outStream.close();
		}
	}
